...
The codes should be run in QGIS IDE. The name of the layers should be set first. There are some QGIS preparation needed before running the codes which are explained below. All the layers have been unified with same geographic extent and coordinate systems. The layers could be inported from csv, or shp, or gpkg files with ground motion data such as those from the European Ground Motion Service (EGMS) or with export commands in MintPy.
